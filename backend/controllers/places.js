...
router.post('/', async (req, res) => {
    if(req.currentUser?.role !== 'admin') {
        return req.status(403).josn({ message: 'You are not allouded to add place'})
    }
    if (!req.body.pic) {
...
router.put('/:placeId', async (req, res) => {
    if(req.currentUser?.role !== 'admin') {
        return req.status(403).josn({ message: 'You are not allouded to edit place'})
    }
    let placeId = Number(req.params.placeId)
    if (isNaN(placeId)) {
        res.status(404).json({ message: `Invalid id "${placeId}"` })
...
router.delete('/:placeId', async (req, res) => {
    if(req.currentUser?.role !== 'admin') {
        return req.status(403).josn({ message: 'You are not allouded to delete place'})
    }
    let placeId = Number(req.params.placeId)
    if (isNaN(placeId)) {
        res.status(404).json({ message: `Invalid id "${placeId}"` })
